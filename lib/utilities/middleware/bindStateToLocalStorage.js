...
const bindStateToLocalStorage = {
            ...
            for (let attr in obj) {
                ...
                if (obj.hasOwnProperty(attr))
                    if (!attr.startsWith('_')) {
                        copy[attr] = bindStateToLocalStorage.removeTemporaryAttr(obj[attr]);
                    }
            }
            return copy;
        }
        ...
        if (obj instanceof Object) {
            ...
            for (let attr in obj) {
                if (obj[attr] != null && typeof obj[attr] == 'object') {
                    let xCopy = bindStateToLocalStorage.removeLocalStoreStates(obj[attr]);
    ...
    bind: reducer => (state, action) => {
        ...
        if (action.type === '@@INIT' || xInit || action.type === 'RESET_STATE') {
    ...
    bindListener: (reducer, pLocalItemPrefix) => (pState, pAction) => {
        ...
        if (pAction.type === '@@INIT' || xInit || pAction.type === 'RESET_STATE') {
            // Inicializa state a partir do reducer
            ...
            xDefaultState = xNewState;
        } else if (pAction.type === 'USER_LOADED' || pAction.type === 'APP_CONNECTION_CHANGED') {
            ...
            if (!validators.isEmpty(xDefaultState)) {
                const { app, auth, message, server, user, ...rest } =
