...
function InfiniteScroller(props) {
    ...
    const prevPageObserver = (pTarget, pScrollElem) => {
        if (pTarget) {
            const xOptions = {
                root: pScrollElem,
    ...
    useEffect(() => {
        if (
            ...
            const xScrollerRect = scroller.current.scrollRef.current.getBoundingClientRect();
    ...
    useEffect(() => {
        if (validators.isNull(props.prevPage) && !validators.isNull(props.onPrevPage)) {
            onMountScroll();
        }

        if (!validators.isNull(props.onPrevPage) && !validators.isNull(props.prevPage)) {
            rightScroll();
        }
