...
const Icon = memo(props => {
    const { color = 'default', size = '24', iconName, className, style } = props;

    const xIconName = '-i_' + iconName;
    ...
    if (validators.isEmpty(iconName)) {
        return null;
    } else {
        return (
            <Svg
                width={isValidSize(size)}
                height={isValidSize(size)}
                className={[xIconName, className].join('')}
                color={color}
                size={size}
                style={style}
                viewBox={'0 0 24 24'}>
